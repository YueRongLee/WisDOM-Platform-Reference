...
import WhereNodeModal from './WhereNodeModal';
import './WhereNodeStyle.less';

export class WhereNodeWidget extends React.Component {
  ...
  handleOk = data => {
    this.props.node.setModalData(data);
  };
  ...
  generatePort = port => {
    const portCom = (
      <PortWidget engine={this.props.engine} port={port} key={port.getID()}>
        <div className="where-port" />
      </PortWidget>
    );
    const label = <div className="where-port-label" />;

    return (
      <div className="where-port-label-container">
      ...
      >
        ...
        >
          <div className="where-title">
            <div className="where-title-name">
          ...
          <div className="where-port-container">
