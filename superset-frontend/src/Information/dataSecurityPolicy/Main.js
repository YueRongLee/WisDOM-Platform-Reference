...
const Main = () => {
  ...
  const saveContent = async () => {
    try {
      await AnnouncementApi.saveContent(INFO_TITLE.dataSecurityPolicy, content);
  ...
  return (
    <div className="dataSecurityPolicy">
      {localStorage.getItem('role').includes(ROLE_TYPE.SYSTEM_MASTER) ? (
        <>
          ...
          >
            <div className="ManaTitle">Data Security Policy</div>
            <div>
              {SYSTEMLIST &&
              ROLEPERMISSION.checkPermission(
                SYSTEMLIST,
                ROLEPERMISSION.information.dataSecurityPolicy.edit.value,
              ) ? (
                <>
                  {!editable && (
                    <Button
