...
const Main = () => {
  ...
  const getContent = async () => {
    try {
      const result = await AnnouncementApi.getContent(
        INFO_TITLE.learningByFunctions,
      );
  ...
  return (
    <div className="learningByFunctions">
      {localStorage.getItem('role').includes(ROLE_TYPE.SYSTEM_MASTER) ? (
        <>
          ...
          >
            <div className="ManaTitle">Learning By Functions</div>
            <div>
              {SYSTEMLIST &&
              ROLEPERMISSION.checkPermission(
                SYSTEMLIST,
                ROLEPERMISSION.information.learningByFunctions.edit.value,
              ) ? (
                <>
                  {!editable && (
                    <Button
