...
const Main = () => {
  ...
  const getContent = async () => {
    try {
      const result = await AnnouncementApi.getContent(INFO_TITLE.workshop);
  ...
  const saveContent = async () => {
    try {
      await AnnouncementApi.saveContent(INFO_TITLE.workshop, content);
    ...
    <div className="workshop">
      {localStorage.getItem('role').includes(ROLE_TYPE.SYSTEM_MASTER) ? (
        <>
          ...
          >
            <div className="ManaTitle">Workshop</div>
            <div>
              {SYSTEMLIST &&
              ROLEPERMISSION.checkPermission(
                SYSTEMLIST,
                ROLEPERMISSION.information.workshop.edit.value,
              ) ? (
                <>
                  {!editable && (
                    <Button
