...
const Main = () => {
  ...
  const getContent = async () => {
    try {
      const result = await AnnouncementApi.getContent(INFO_TITLE.sitemap);
  ...
  const saveContent = async () => {
    try {
      await AnnouncementApi.saveContent(INFO_TITLE.sitemap, content);
    ...
    <div className="sitemap">
      {localStorage.getItem('role').includes(ROLE_TYPE.SYSTEM_MASTER) ? (
        <>
          ...
          >
            <div className="ManaTitle">Site map</div>
            <div>
              {SYSTEMLIST &&
              ROLEPERMISSION.checkPermission(
                SYSTEMLIST,
                ROLEPERMISSION.information.siteMap.edit.value,
              ) ? (
                <>
                  {!editable && (
                    <Button
