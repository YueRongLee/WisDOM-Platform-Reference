...
const Main = () => {
  ...
  const getContent = async () => {
    try {
      const result = await AnnouncementApi.getContent(
        INFO_TITLE.learningByScenarios,
      );
  ...
  const saveContent = async () => {
    try {
      await AnnouncementApi.saveContent(
        INFO_TITLE.learningByScenarios,
        content,
      );
  ...
  return (
    <div className="learningByScenarios">
      {localStorage.getItem('role').includes(ROLE_TYPE.SYSTEM_MASTER) ? (
        <>
          ...
          >
            <div className="ManaTitle">Learning By Scenarios</div>
            <div>
              {SYSTEMLIST &&
              ROLEPERMISSION.checkPermission(
                SYSTEMLIST,
                ROLEPERMISSION.information.learningByScenarios.edit.value,
              ) ? (
                <>
                  {!editable && (
                    <Button
