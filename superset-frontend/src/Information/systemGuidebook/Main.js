...
const Main = () => {
  ...
  const saveContent = async () => {
    try {
      await AnnouncementApi.saveContent(INFO_TITLE.systemGuidebook, content);
    ...
    <div className="systemGuidebook">
      {localStorage.getItem('role').includes(ROLE_TYPE.SYSTEM_MASTER) ? (
        <>
          ...
          >
            <div className="ManaTitle">System Guidebook</div>
            <div>
              {SYSTEMLIST &&
              ROLEPERMISSION.checkPermission(
                SYSTEMLIST,
                ROLEPERMISSION.information.systemGuidebook.edit.value,
              ) ? (
                <>
                  {!editable && (
                    <Button
