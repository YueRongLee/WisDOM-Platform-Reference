...
export function runQuery(query) {
  return function (dispatch) {
    ...
    const userRoles = sessionStorage.getItem('userRoles')
      ...
      : [];
    if (!userRoles.includes(ROLE_TYPE.DATA_MASTER)) {
      return SupersetClient.post({
        body: JSON.stringify({ sql: postPayload.sql }),
        headers: {
        ...
        parseMethod: 'text',
        url: `${AppConfig.serverUrl}/table/auth`,
        mode: 'cors',
      })
        .then(() =>
          SupersetClient.post({
            ...
            .catch(response =>
              getClientErrorObject(response).then(error => {
                let message =
                  error.error || error.statusText || t('Unknown error');
        ...
        .catch(response =>
          getClientErrorObject(response).then(error => {
            const message =
              response.status === 401 ? 'No permission' : t('Unknown error');
            dispatch(queryFailed(query, message, error.link, error.errors));
          }),
        );
    }
